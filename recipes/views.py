...
def subcategory_recipes(request, subcategory_id):
    ...
    return render(request, "foodie_app/recipes_subcategory.html", context)
...
def recipe_detail(request, recipe_id):
    ...
    comments = (
        recipe.comments.all()
    )  # Obtener todos los comentarios relacionados con la receta
    new_comment = None
    ...
    if request.method == "POST":
        ...
        if comment_form.is_valid():
            ...
            new_comment.user = (
                request.user
            )  # Asignar el usuario autenticado al comentario
            new_comment.save()
...
def favorite_recipes(request):
    ...
    if user.is_authenticated:
        ...
        favorite_recipes = (
            Recipe.objects.none()
        )  # Retorna un queryset vacío si el usuario no está autenticado
